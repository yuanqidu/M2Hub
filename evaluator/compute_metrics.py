...
from eval_utils import (
    ...
    load_data, get_crystals_list, prop_model_eval, compute_cov)
...
class GenEval(object):
    ...
    def __init__(self, pred_crys, gt_crys, n_samples=1000, eval_model_name=None, COV_Cutoffs=None):
        self.crys = pred_crys
        self.gt_crys = gt_crys
        self.n_samples = n_samples
        self.eval_model_name = eval_model_name
        self.COV_Cutoffs = COV_Cutoffs
...
class OptEval(object):
    ...
    def __init__(self, crys, num_opt=100, eval_model_name=None, Percentiles=None):
        """
...
def main(args):
    ...
    if 'gen' in args.tasks:
        ...
        gen_evaluator = GenEval(
            gen_crys, gt_crys, eval_model_name=eval_model_name, COV_Cutoffs=args.COV_Cutoffs)
    ...
    if 'opt' in args.tasks:
        ...
        opt_evaluator = OptEval(opt_crys, eval_model_name=eval_model_name, Percentiles=args.Percentiles)
...
if __name__ == '__main__':
    Percentiles = {'my_model': np.array([0.43924842, 0.61202443, 0.7364607])}
    COV_Cutoffs = {'my_model': {'struc': 0.2, 'comp': 4}}
    parser = argparse.ArgumentParser()
    parser.add_argument('--root_path', default='/my_data', required=True)
    parser.add_argument('--eval_model_name', default='my_model', required=True)
    parser.add_argument('--label', default='')
    parser.add_argument('--tasks', nargs='+', default=['recon', 'gen', 'opt'])
    args = parser.parse_args()
    args.Percentiles = Percentiles['my_model']
    args.COV_Cutoffs = COV_Cutoffs['my_model']
    main(args)