...
from data_utils import StandardScaler, chemical_symbols, TensorCrystDataset, worker_init_fn
...
def get_model_path(eval_model_name):
    model_path = os.path.dirname(os.path.abspath(__file__)) + '/prop_models/' +  eval_model_name
    return model_path
...
def load_model(model_path):
    with initialize_config_dir(str(model_path)):
        ...
        ckpts = [os.path.join(model_path, f) for f in os.listdir(model_path) if f.endswith('.ckpt')]
        # ckpts = list(model_path.glob('*.ckpt'))
        if len(ckpts) > 0:
            # ckpt_epochs = np.array(
            #     [int(ckpt.parts[-1].split('-')[0].split('=')[1]) for ckpt in ckpts])
            # ckpt = str(ckpts[ckpt_epochs.argsort()[-1]])
            ckpt_epochs = np.array([int(ckpt.split('-')[0].split('=')[1]) for ckpt in ckpts])
            ckpt = str(ckpts[ckpt_epochs.argsort()[-1]])
