...
from m2models.common.utils import collate
...
class AtomsToGraphs:
    """A class to help convert (non-periodic) atomic structures to graphs.

    The AtomsToGraphs class takes in (non-periodic) atomic structures in form of rdkit mol objects and converts
    them into graph representations for use in PyTorch. The primary purpose of this class is to determine the
    neighbors within some radius around each individual atom, and set the pair index and distance between atom pairs
    appropriately. Lastly, atomic properties and the graph information are put into a PyTorch geometric data object for use with PyTorch.
    ...
    def convert(
        ...
        Args:
            mols (rdkit.rdmol): An rdmol molecule object.

        Returns:
            data (torch_geometric.data.Data): A torch geometic data object with positions, atomic_numbers, tags,
            and optionally, energy, forces, distances and edges.
        """

        # set the atomic numbers, positions, and cell
        atomic_numbers = torch.Tensor([atom.GetAtomicNum() for atom in mol.GetAtoms()])
        positions = torch.Tensor([mol.GetConformer(0).GetAtomPosition(idx) for idx in range(mol.GetNumAtoms())])
        natoms = positions.shape[0]
        ...
        adj = torch.cdist(positions, positions) <= self.radius 
