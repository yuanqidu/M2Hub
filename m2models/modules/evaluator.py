...
```
from m2models.modules import Evaluator
...
class Evaluator:
    ...
    def __init__(self, task=None):
        assert task in ["s2ef", "is2rs", "is2re", "classification"]
        self.task = task
...
def roc_auc(prediction, target):
    from sklearn.metrics import roc_auc_score
    # readout = torch.nn.Softmax(dim=-1)
    # prediction = readout(prediction["energy"])
    # prediction = prediction.argmax(dim=-1)
    acc = roc_auc_score(target["energy"].detach().cpu().numpy(), prediction.detach().cpu().numpy())
    return -acc
